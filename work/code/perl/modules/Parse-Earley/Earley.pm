...
=head1 SYNOPSIS
  ...
  while (1) {
    ...
    $parser->advance($str);
    die "Parse failed" if $parser->fails($str, 'mystartrule');
	if (($graph) = $parser->matches($str, 'mystartrule')) {
      print "Parse succeeded\n";
      last;
	}
  }

  # Do stuff with the parse graph $graph
...
The methods C<matches> and C<fails> check whether the parser accepts or rejects
the input. Note that these can both be false (though they cannot both be true),
and always are in the middle of parsing.  C<matches_all> checks whether the
input match, and that no more parses are still possible; C<matches_all> is
more restrictive than C<matches>.

The C<matches> and C<matches_all> functions, if true, return a list of complete
...
parser state with the C<matches> function. C<matches> returns an array, though
there's usually only one such state.  So, just listify your variable:

  ($graph) = $parser->matches($input, 'mystartrule');

or

  ($graph) = $parser->matches_all($input, 'mystartrule');
...
A <commit> directive, to remove extraneous states if the current one matched.
